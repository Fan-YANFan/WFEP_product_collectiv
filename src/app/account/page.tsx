...
import { Bell, Bookmark, CalendarPlus, ClipboardList, type LucideIcon } from "lucide-react";
...
function AccountSection({ id, icon: Icon, iconWrapClass, title, description, children }: AccountSectionProps) {
  return (
    <section
      id={id}
      className="scroll-mt-24 rounded-2xl border border-slate-200/90 bg-white px-6 py-7 shadow-sm sm:px-8 sm:py-8"
    >
      <header className="mb-7 flex gap-4 border-b border-slate-100 pb-6">
        <div
          className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${iconWrapClass}`}
        >
    ...
    <>
      ...
      <div className="bg-slate-50/80">
        <div className="mx-auto max-w-6xl space-y-8 px-4 py-10 sm:space-y-10 sm:px-6 sm:py-14">
          ...
          <AccountSection
            icon={ClipboardList}
            iconWrapClass="bg-brand-cyan-muted text-brand-cyan-foreground"
            title={t.account.orderHistory}
            description={t.account.orderDesc}
          >
            {orders.length === 0 ? (
          ...
          <AccountSection
            id="reminders"
            icon={Bell}
            iconWrapClass="bg-violet-100 text-violet-800"
            title={t.account.reminders}
            description={t.account.remindersDesc}
          >
            <ReminderCountdownCards reminders={reminders} />

            <form
              onSubmit={handleAddReminder}
              className="mt-6 rounded-2xl border border-slate-200 bg-slate-50/60 p-6"
            >
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="sm:col-span-2">
                  ...
                  <input
                    ...
                    className="input-brand mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm"
                  />
                </div>
                <div>
            ...
            {reminders.length === 0 ? (
              ...
              <ul className="mt-6 space-y-3">
                {reminders.map((reminder) => (
                  <li
                    key={reminder.id}
                    className="flex flex-wrap items-start justify-between gap-3 rounded-xl border border-slate-200 bg-white px-5 py-4 shadow-sm"
                  >
                    <div>
